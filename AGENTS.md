...
├── EVAL_PROTOCOL.md       # 测试与验证协议（论文方法学）⭐
├── data/
...
- v1.1 代码开发完成，待训练验证
- **建立测试验证协议**：创建 `EVAL_PROTOCOL.md`，定义论文可用的真实场景测试标准流程
- **待解决**：二分类需要准备阴性样本
...
### 作用
定义**模型在真实场景下的测试与验证标准流程**，可直接写入论文方法学部分，确保结果可复现、可量化。

### 核心内容
| 章节 | 说明 |
|------|------|
| 测试集规模 | 主实验10-50万条，大规模验证100万条（拒绝3亿条全量测试） |
| Silver Standard | 用高置信同源比对（DIAMOND/MMseqs2）构建参考标签 |
| 防泄漏机制 | 同源去重（pident<90%）、按来源holdout |
| 阈值选择 | 验证集最优阈值 vs 固定0.5，及敏感性分析 |
| 评估指标 | 二分类（F1/Recall/Specificity）、多分类（Macro-F1）、端到端错误分解 |
| SOTA对比 | 必须与RGI/DeepARG对比，报告统计显著性 |

### 使用时机
- **模型训练完成后**：按照此协议进行真实数据测试
- **论文撰写时**：方法学部分直接引用此文档内容
- **审稿回复时**：证明测试流程的严谨性和可复现性
